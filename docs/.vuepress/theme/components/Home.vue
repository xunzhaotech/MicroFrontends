...
<script>
import { ensureExt, Rain, Star, draw } from '../util'
...
export default {
  ...
  methods:{
    windowResizeHandler() {
      ...
      this.starCount = this.canvasSize.width * this.starDensity;
      this.circleRadius = (this.canvasSize.width > this.canvasSize.height ? this.canvasSize.height / 2 : this.canvasSize.width / 2);
      this.circleCenter = {
        x: this.canvasSize.width / 2,
        y: this.canvasSize.height / 2
      }
      this.canva.height = this.canvasSize.height
      this.canva.width = this.canvasSize.width
    },
    createUniverse() {
      ...
      for (let i = 0; i < this.starCount; i++) {
        this.stars[i] = new Star(this.universe, this.canvasSize.width, this.canvasSize.height, this.first, this.speedCoeff, this.giantColor, this.starColor, this.cometColor);
        this.stars[i].reset();
      }
     this.draw();
    },
    draw() {
      this.universe.clearRect(0, 0, this.canvasSize.width, this.canvasSize.height);
    ...
    createRain() {
        const rain = new Rain(this.universe, this.canvasSize.width, this.canvasSize.height)
        rain.init() //基本配置
        rain.draw()
        this.rainArray.push(rain)
    },
    moveRain() {
        this.universe.fillStyle = "rgba(0, 0, 0, 0.05)"
        this.universe.fillRect(0, 0, this.canvasSize.width, this.canvasSize.height)
  ...
  mounted() {
    ...
    this.canva = this.$refs.universe;
     // 获取浏览器高度和宽度
    this.canva.setAttribute('width', this.canvasSize.width);
    this.canva.setAttribute('height', this.canvasSize.height);
    this.createUniverse();
    this.createRain();
    for (let i = 0; i < 30; i++) {
